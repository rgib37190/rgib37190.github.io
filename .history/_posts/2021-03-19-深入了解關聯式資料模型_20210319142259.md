...
$$D=dom(A1)\cup dom(A2)\cup….dom\cup(An)從笛卡爾積換成聯集$$
...
               當插入的值不在定義域里時→違反domain constraint

               當插入的值的key已經存在本來的資料表裡的時候→違反key constraint

               當插入的值的主鍵為NULL時→違反integrity constraint

               當插入的值的外鍵無法在被參考的資料表中找到相關的資料→違反referential constraint
